...
from .utils import HELP_URL, LOGGER, get_hash, img2label_paths, IMG_FORMATS, verify_image_label
...
class BaseDataset(Dataset):
    ...
    def load_image(self, i, rect_mode=True):
        ...
        if im is None:  # not cached in RAM
            ...
            if self.augment:
                self.ims[i], self.im_hw0[i], self.im_hw[i] = im, (h0, w0), im.shape[
...
class CombinedDataset(Dataset):
    def __init__(self, ir_dataset, irvis_dataset):
        super().__init__()
        self.ir_dataset = ir_dataset
        self.irvis_dataset = irvis_dataset

    def __getitem__(self, index):
        data_type, idx = index
        if data_type == 'ir':
            return self.ir_dataset[idx]
        else:
            return self.irvis_dataset[idx]
