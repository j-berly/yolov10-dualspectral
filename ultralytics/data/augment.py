...
class BaseMixTransform:
    ...
    def __call__(self, labels):
        ...
        indexes = [
            random.choices(list(
                filter(lambda x: os.path.dirname(self.dataset.im_files[x]) == os.path.dirname(labels['im_file']),
                       range(len(self.dataset.im_files)))))
...
class Mosaic(BaseMixTransform):
        ...
        for i in range(3):
            ...
            if i == 0:  # center
                if isinstance(img, tuple) or isinstance(img, list):
                    img3 = [np.full((s * 3, s * 3, _im.shape[2]), 114, dtype=np.uint8) for _im in
                            img]  # base image with 3 tiles
                else:
            ...
            if isinstance(img, tuple) or isinstance(img, list):
                img3[0][y1:y2, x1:x2] = img[1][y1 - padh:, x1 - padw:]  # img3[ymin:ymax, xmin:xmax]
                img3[0][y1:y2, x1:x2] = img[1][y1 - padh:, x1 - padw:]  # img3[ymin:ymax, xmin:xmax]
            else:
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                if isinstance(img, tuple) or isinstance(img, list):
                    img4 = [np.full((s * 2, s * 2, _im.shape[2]), 114, dtype=np.uint8) for _im in
                            img]  # base image with 4 tiles
                else:
            ...
            if isinstance(img, (tuple, list)):
                img4[0][y1a:y2a, x1a:x2a] = img[0][y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
                img4[1][y1a:y2a, x1a:x2a] = img[1][y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
            else:
    ...
    def _mosaic9(self, labels):
        ...
        for i in range(9):
            ...
            if i == 0:  # center
                if isinstance(img, tuple) or isinstance(img, list):
                    img9 = [np.full((s * 3, s * 3, _im.shape[2]), 114, dtype=np.uint8) for _im in
                            img]  # base image with 4 tiles
                else:
            ...
            # Image
            if isinstance(img, tuple) or isinstance(img, list):
                img9[0][y1:y2, x1:x2] = img[0][y1 - padh:, x1 - padw:]  # img9[ymin:ymax, xmin:xmax]
                img9[1][y1:y2, x1:x2] = img[1][y1 - padh:, x1 - padw:]  # img9[ymin:ymax, xmin:xmax]
            else:
        ...
        if isinstance(img, tuple) or isinstance(img, list):
            final_labels["img"] = [_im[-self.border[0]: self.border[0], -self.border[1]: self.border[1]] for _im in img9]
        else:
    ...
    def _update_labels(labels, padw, padh):
        """Update labels."""
        if isinstance(labels["img"], list) or isinstance(labels["img"], tuple):
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        if isinstance(labels["img"], list) or isinstance(labels["img"], tuple):
            labels["img"] = [(_im1 * r + _im2 * (1 - r)).astype(np.uint8) for _im1, _im2 in zip(labels['img'], labels2['img'])]
        else:
...
class RandomPerspective:
    ...
    def affine_transform(self, img, border):
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                if isinstance(img, tuple) or isinstance(img, list):
                    img = [cv2.warpPerspective(_im, M, dsize=self.size, borderValue=(114, 114, 114)) for _im in img]
                else:
                    img = cv2.warpPerspective(img, M, dsize=self.size, borderValue=(114, 114, 114))
            else:  # affine
                if isinstance(img, tuple) or isinstance(img, list):
                    img = [cv2.warpAffine(_im, M[:2], dsize=self.size, borderValue=(114, 114, 114)) for _im in img]
                else:
    ...
    def __call__(self, labels):
        ...
        if isinstance(img, tuple) or isinstance(img, list):
            instances.denormalize(*img[0].shape[:2][::-1])
        else:
            instances.denormalize(*img.shape[:2][::-1])

        border = labels.pop("mosaic_border", self.border)
        if isinstance(img, tuple) or isinstance(img, list):
            self.size = img[0].shape[1] + border[1] * 2, img[0].shape[0] + border[0] * 2  # w, h
        else:
        ...
        labels["cls"] = cls[i]
        labels["img"] = img
        labels["resized_shape"] = img[0].shape[:2] if isinstance(img, tuple) or isinstance(img, list) else img.shape[:2]
        return labels
        ...
        if self.hgain or self.sgain or self.vgain:
            if isinstance(img, list) or isinstance(img, tuple):
                for _im in img:
                    ...
                    hue, sat, val = cv2.split(cv2.cvtColor(_im, cv2.COLOR_BGR2HSV))
                    dtype = _im.dtype  # uint8
                    ...
                    cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR, dst=_im)  # no return needed
            else:
                r = np.random.uniform(-1, 1, 3) * [self.hgain, self.sgain, self.vgain] + 1  # random gains
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        h, w = img[0].shape[:2] if isinstance(img, tuple) or isinstance(img, list) else img.shape[:2]
        h = 1 if instances.normalized else h
        w = 1 if instances.normalized else w

        # Flip up-down
        if self.direction == "vertical" and random.random() < self.p:
            img = [np.flipud(_im) for _im in img] if isinstance(img, tuple) or isinstance(img, list) else np.flipud(img)
            instances.flipud(h)
        if self.direction == "horizontal" and random.random() < self.p:
            img = [np.fliplr(_im) for _im in img] if isinstance(img, tuple) or isinstance(img, list) else np.fliplr(img)
            instances.fliplr(w)
            # For keypoints
            if self.flip_idx is not None and instances.keypoints is not None:
                instances.keypoints = np.ascontiguousarray(instances.keypoints[:, self.flip_idx, :])
        labels["img"] = [np.ascontiguousarray(_im) for _im in img] if isinstance(img, tuple) or isinstance(img, list) else np.ascontiguousarray(img)
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        shape = img[0].shape[:2] if isinstance(img, tuple) or isinstance(img, list) else img.shape[
                                                                                         :2]  # current shape [height, width]
        ...
        if shape[::-1] != new_unpad:  # resize
            if isinstance(img, tuple) or isinstance(img, list):
                img = [cv2.resize(_im, new_unpad, interpolation=cv2.INTER_LINEAR) for _im in img]
            else:
        ...
        if isinstance(img, tuple) or isinstance(img, list):
            img = [cv2.copyMakeBorder(
                _im, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
            ) for _im in img]  # add border
        else:
            img = cv2.copyMakeBorder(
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        im = labels["img"]
        cls = labels["cls"]
        h, w = im[0].shape[:2] if isinstance(im, (list, tuple)) else im.shape[:2]
        instances = labels.pop("instances")
        instances.convert_bbox(format="xyxy")
        instances.denormalize(w, h)
        if self.p and len(instances.segments):
            n = len(instances)
            _, w, _ = im[0].shape if isinstance(im, (list, tuple)) else im.shape  # height, width, channels
            im_new = np.zeros(im[0].shape if isinstance(im, (list, tuple)) else im.shape, np.uint8)
            ...
            if isinstance(im, (list, tuple)):
                result = cv2.flip(im[0], 1)  # augment segments (flip left-right)
...
class Albumentations:
    ...
    def __call__(self, labels):
        ...
        if len(cls):
            ...
            if isinstance(im, (list, tuple)):
                labels["instances"].normalize(*im[0].shape[:2][::-1])
            else:
            ...
            if self.transform and random.random() < self.p:
                if isinstance(im, (list, tuple)):
                    labels["cls"] = labels["cls"].squeeze(-1)
                else:
...
class Format:
    ...
    def __call__(self, labels):
        ...
        img = labels.pop("img")
        h, w = img[0].shape[:2] if isinstance(img, tuple) or isinstance(img, list) else img.shape[
                                                                                        :2]  # current shape [height, width]
        cls = labels.pop("cls")
        instances = labels.pop("instances")
        ...
        if self.return_mask:
            if nl:
                ...
                if isinstance(img, tuple) or isinstance(img, list):
                    masks = torch.zeros(
                        1 if self.mask_overlap else nl, img[0].shape[0] // self.mask_ratio, img[0].shape[1] // self.mask_ratio
                    )
                else:
                    masks = torch.zeros(
    ...
    def _format_img(self, img):
        ...
        if isinstance(img, tuple) or isinstance(img, list):
            for i, _im in enumerate(img):
                if len(_im.shape) < 3:
                    _im = np.expand_dims(_im, -1)
                _im = _im.transpose(2, 0, 1)
                _im = np.ascontiguousarray(_im[::-1] if random.uniform(0, 1) > self.bgr else _im)
                img[i] = torch.from_numpy(_im)
            return img
        else:
            if len(img.shape) < 3:
