...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            if isinstance(m, v10Detect) and any('FusionConv' in m.type for m in self.model):
...
class YOLOv10DetectionModel(DetectionModel):
    ...
    def _register_hooks(self):
        """为指定层注册hook"""
        target_layers = [
            layer for name, layer in self.model.named_modules()
            if isinstance(layer, (Conv, C2f, SPPF))  # 选择关注的层类型
        ]
        for layer in target_layers:
            handle = layer.register_forward_hook(
                partial(self._hook_fn, layer_name=layer.__class__.__name__))
