...
class GradientWatcher:
    def __init__(self, model, save_dir="./grad_plots", dpi=150, show_plot=False):
        self.model = model
    ...
    def plot(self, fname):
        ...
        if gradients:
            ...
            self.ax[0].hist(gradients, bins=100, range=(-1e-3, 1e-3), log=True)
            self.ax[0].set_title(f'Gradient Distribution (Step {self.step})')
        ...
        if params:
            ...
            n, bins, patches = self.ax[1].hist(params, bins=100, density=True, alpha=0.7)

            # 添加高斯拟合
            mu, sigma = np.mean(params), np.std(params)
            y = ((1 / (np.sqrt(2 * np.pi) * sigma)) *
                 np.exp(-0.5 * (1 / sigma * (bins - mu)) ** 2))
            self.ax[1].plot(bins, y, 'r--')
            self.ax[1].set_title(f'Parameter Distribution (μ={mu:.2e}, σ={sigma:.2e})')
...
def visualize_features(feature_maps, epoch, save_dir='./visualization'):
    save_dir = Path(save_dir) / "features" / f"epoch_{epoch}"
    save_dir.mkdir(parents=True, exist_ok=True)

    for name, feat in feature_maps.items():
        np.save(save_dir / f"{name}.npy", feat)


def save_feature_as_image(feature_path, channel_idx=None):
    """
    将特征图保存为图像

    Args:
        feature_path (str): 特征图数组，形状为 [batch, channels, H, W]
        channel_idx (int/list): 可选，指定通道索引（默认所有通道平均值）
    """
    # 选取第一个样本
    feature = np.load(Path(feature_path))[0]

    # 通道处理
    if channel_idx is not None:
        if isinstance(channel_idx, int):
            feature = feature[channel_idx]
        else:
            feature = feature[channel_idx].mean(axis=0)
    else:
        feature = feature.mean(axis=0)  # 通道维度取平均

    # 归一化到0-255
    feat_norm = cv2.normalize(feature.astype(np.float32), None, 0, 255, cv2.NORM_MINMAX)
    feat_uint8 = feat_norm.astype(np.uint8)

    # 保存图像
    cv2.imwrite(feature_path.replace('npy', 'jpg'), cv2.resize(feat_uint8, (256, 256), interpolation=cv2.INTER_CUBIC))


def save_rgb_feature(feature, save_path):
    """将特征图转为伪RGB图像"""
    # 选取前3个通道
    rgb_feat = feature[:3].transpose(1, 2, 0)  # [3, H, W] -> [H, W, 3]

    # 归一化处理
    rgb_norm = (rgb_feat - rgb_feat.min()) / (rgb_feat.max() - rgb_feat.min()) * 255
    cv2.imwrite(save_path, rgb_norm.astype(np.uint8))

def save_heatmap(feature, save_path):
    """生成热力图"""
    feat_mean = feature.mean(axis=0)
    heatmap = cv2.applyColorMap(feat_mean.astype(np.uint8), cv2.COLORMAP_JET)
